...
def inspect_file(
    ...
    if source.stat().st_size == 0:
        return QcReport(
            ...
            dynamic_range_db=0,
            speckle_index=0,
            dropout_ratio_percent=0,
            motion_artifact_rows=[],
            resolution_meters_per_pixel=0.1,
            status="CORRUPTED",
            recommendations=["The uploaded file is empty."],
        ), None
    ...
    if extraction is not None:
        ...
        motion_rows = np.where(
            np.abs(row_means - row_means.mean()) > 3 * local_std
        )[0].astype(int).tolist()
        dropout = round(100 * len(motion_rows) / max(len(row_means), 1), 2)
        dynamic_range = round(
            float(np.percentile(pixels, 99) - np.percentile(pixels, 1)), 2
        )
        speckle = round(float(pixels.std() / max(pixels.mean(), 1.0)), 3)
        pings = int(pixels.shape[0])

    status = "WARNING" if dropout > 3 or pings == 0 else "PASS"
    recommendations: list[str] = []
    if pings == 0:
        recommendations.append(
            "Binary metadata extraction is pending; processing will refuse geolocation."
        )
    if dropout > 3:
        recommendations.append("Exclude motion-artifact rows from detection tiles.")
    if not recommendations:
        recommendations.append(
            "QC passed; preserve raw intensity for detection (DSP remains display-only)."
        )
...
def _navigation_for_row(
    extraction: dict | None, row: float, image_height: int
) -> dict | None:
    navigation = (extraction or {}).get("navigation", [])
    if not navigation:
        return None
    target = row / max(image_height, 1) * max(len(navigation) - 1, 0)
    index = min(max(int(round(target)), 0), len(navigation) - 1)
...
def _iter_yolo_pipeline(
    ...
    resolution = float(qc["resolution_meters_per_pixel"])
    model_path = Path(adapter.config.model_path)
    model_digest = _sha256_file(model_path) if model_path.is_file() else None

    for tiles, tile_metadata in _iter_tile_batches(
        image, tile_size=tile_size, batch_size=batch_size
    ):
        batch_results = adapter.run_batch(tiles, resolution_m_per_px=resolution)
        for tile_result, (left, top, _crop_width, _crop_height) in zip(
            batch_results, tile_metadata
        ):
            for raw in tile_result:
                center_x = left + (
                    raw.x_norm + raw.box_xywh_norm[2] / 2
                ) * tile_size
                center_y = top + (
                ...
                timestamp = (
                    navigation.get("timestamp") if navigation else None
                ) or datetime.now(timezone.utc).isoformat()
                ping_index = int(
                    navigation.get("ping_index", round(center_y))
                    if navigation
                    else round(center_y)
                )
                yield {
                    ...
                    "bounding_box": {
                        "x": round(
                            min(max(center_x / max(image_width, 1), 0), 1), 6
                        ),
                        "y": round(
                            min(max(center_y / max(image_height, 1), 0), 1), 6
                        ),
                        "width_m": round(raw.width_m, 3),
                        "height_m": round(raw.height_m, 3),
                    },
                    "segmentation_mask": raw.seg_mask,
                    "position": position,
                    "calibrated": False,
                    "low_data_quality": bool(qc.get("motion_artifact_rows")),
                    "motion_uncorrected": not bool(
                        navigation
                        and navigation.get("pitch_deg") is not None
                        and navigation.get("roll_deg") is not None
                    ),
                    "model_version": raw.model_version,
                    "dsp_applied": dsp_applied,
                    "ping_timestamp": timestamp,
                    "ping_index": max(ping_index, 0),
...
def _iter_heuristic_fallback(
    ...
    nav = (
        nav_points[
            min(len(nav_points) - 1, int(len(nav_points) * rng.uniform()))
        ]
        if nav_points
        else None
    )
    ...
    position = (
        ...
        if valid_fix
            ...
            "refusal_reason": "Valid navigation metadata was not available for this ping.",
        }
    )
    yield {
        "id": str(uuid4()),
        ...
        "bounding_box": {
            ...
            "width_m": round(float(rng.uniform(0.4, 2.0)), 2),
            "height_m": round(float(rng.uniform(0.3, 1.4)), 2),
        },
        "segmentation_mask": None,
        "position": position,
        "calibrated": False,
        "low_data_quality": bool(qc.get("motion_artifact_rows")),
        "motion_uncorrected": True,
        "model_version": "heuristic-baseline-v1 (model unavailable)",
        "dsp_applied": dsp_applied,
        ...
        "feature_weights": {
            "target_contrast": 0.24,
            "shadow_ratio": 0.17,
            "local_snr": 0.21,
            "background_roughness": -0.12,
        },
        "provenance": {
...
def iter_pipeline(
    ...
    adapter = get_adapter()
    if adapter.is_ready:
        yield from _iter_yolo_pipeline(
            survey_id,
            source,
            qc,
            dsp_applied,
    ...
    yield from _iter_heuristic_fallback(
        survey_id, source, qc, dsp_applied, extraction, source_digest
    )
